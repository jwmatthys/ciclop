...
public class oobleck_user extends PApplet {
...
public void draw() {
  ...
  if (mouseDown==1)
  {
    ...
    myMessage.add(PApplet.parseFloat(mouseX)/width);
    myMessage.add(PApplet.parseFloat(mouseY)/height);
...
public void servMessage(String s)
{
  serverMessage = s;
...
public void hose(String s)
{
  println("host ip: "+s);
  myBroadcastLocation = new NetAddress(s,32000);
  String[] pieces = split(s,".");
  add1.setValue(PApplet.parseInt(pieces[0]));
  add2.setValue(PApplet.parseInt(pieces[1]));
  add3.setValue(PApplet.parseInt(pieces[2]));
  add4.setValue(PApplet.parseInt(pieces[3]));
  controlWindow.hide();
  showWindow = false;
  OscMessage m = new OscMessage("/server/connect",new Object[0]);
  oscP5.flush(m,myBroadcastLocation);
...
public void stop()
{
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#DFDFDF", "oobleck_user" });
  }
}
